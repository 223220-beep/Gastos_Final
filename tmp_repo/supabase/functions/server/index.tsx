...
// Get analytics data
app.get("/make-server-aa79c66d/expenses/analytics", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    ...
    // Calculate monthly totals (last 12 months)
    const now = new Date();
    const monthlyData = [];
    ...
    // Calculate category totals (current month)
    const currentMonthStart = new Date(now.getFullYear(), now.getMonth(), 1).toISOString();
    const currentMonthExpenses = expenses.filter(expense => expense.date >= currentMonthStart);
...
// Get all reminders
app.get("/make-server-aa79c66d/reminders", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    ...
    return c.json({ error: "Error al obtener recordatorios" }, 500);
  }
});

// Create reminder
app.post("/make-server-aa79c66d/reminders", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    ...
    const { description, amount, category, reminderDate } = await c.req.json();

    // Validate required fields
    if (!description || !amount || !category || !reminderDate) {
      return c.json({ error: "Todos los campos son obligatorios" }, 400);
    }
    ...
    return c.json({ error: "Error al crear recordatorio" }, 500);
  }
});

// Update reminder (mark as completed)
app.put("/make-server-aa79c66d/reminders/:id", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    ...
    const existingReminder = await kv.get(`reminder:${user.id}:${reminderId}`);

    if (!existingReminder) {
      return c.json({ error: "Recordatorio no encontrado" }, 404);
    }
    ...
    await kv.set(`reminder:${user.id}:${reminderId}`, updatedReminder);
    ...
    return c.json({ error: "Error al actualizar recordatorio" }, 500);
  }
});

// Delete reminder
app.delete("/make-server-aa79c66d/reminders/:id", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    ...
    return c.json({ message: "Recordatorio eliminado exitosamente" });
  } catch (error) {
    console.log(`Error deleting reminder: ${error}`);
    return c.json({ error: "Error al eliminar recordatorio" }, 500);
  }
});
